...
<script type="text/javascript">
	function _go() {
		...
		location = "${pb.url}&pc="+pc+"&bookingNo=${bookingNo}&date=${date}&storeName=${storeName}&status=${status}";
	}
</script>
...
<div class="divBody">
	<div class="divContent">
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="${pb.url }&pc=${pb.pc-1}&bookingNo=${bookingNo}&date=${date}&storeName=${storeName}&status=${status}" class="aBtn bold">上一页</a>
			</c:otherwise>
		</c:choose>
		...
		<c:forEach begin="${begin }" end="${end }" var="i">
			<c:choose>
				...
				<c:otherwise>
					<a href="${pb.url }&pc=${i}&bookingNo=${bookingNo}&date=${date}&storeName=${storeName}&status=${status}" class="aBtn">${i }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="${pb.url }&pc=${pb.pc+1}&bookingNo=${bookingNo}&date=${date}&storeName=${storeName}&status=${status}" class="aBtn bold">下一页</a>
