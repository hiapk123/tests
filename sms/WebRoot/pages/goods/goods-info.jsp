		...
		<div class="panel panel-default">
			<div class="panel-footer">
				<div class="row">
					...
					<div class="col-xs-1">
						<a onclick="xinzengshangpin()" data-toggle="modal"
							data-target="#myModal">新增商品</a>
					</div>
					<div class="col-xs-1">
						<a data-toggle="modal" data-target="#myModal1" onclick="DaoRu()">导入</a>
					</div>
					<div class="col-xs-1">
						<a data-toggle="modal" data-target="#myModal3" onclick="daochu()">导出</a>
					</div>
					<div class="col-xs-1">
						<a data-toggle="modal" data-target="#myModal4"
							onclick="fuzhishangpin()">复制商品</a>
					</div>
					<div class="col-xs-1">
						<a id="kuaisu" name="submit">快速录入</a>
					</div>
					...
					<table
						...
						<thead>
							<tr class="default">
								...
								<th>商品条码</th>
								<th>库存量
									<button onclick="UP(this)" class=" btn btn-success btn-xs"
										value="g_stock_num">&uarr;</button>
									<button onclick="DOWN(this)" class=" btn btn-success btn-xs"
						...
						<tbody>
							...
							<tr class="default">
								<td><a onclick="del()">删除</a> <a data-toggle="modal"
									data-target="#myModal2" onclick="edit(this)">编辑</a> <input
									type="hidden" class="list" value="<%=list%>"> <%
 	for (int j = 1; j < 9; j++) {
 				if (String.valueOf(goods.get(i)[j]).equals("null"))
 					goods.get(i)[j] = "";
 %>
								<td><%=goods.get(i)[j]%></td>
								<%
								...
								%>
								<td><a data-toggle="modal" data-target="#myModal22"
									onclick="detail(this)">详细</a></td>
					...
					<input type="hidden" id="s_id" value="<%=s_id%>" /> <input
						type="hidden" id="key" value="<%=key%>" /> <input type="hidden"
						id="fenlei" value="<%=c_name%>" /> <input type="hidden"
						id="g_del" value="<%=g_del%>" /> <input type="hidden" id="method"
						value="${method }" /> <input type="hidden" id="sorted"
						value="${sorted }" /> <input type="hidden" id="currentPage"
						value="<%=currentPage%>" />
					<ul class="pagination" id="page"
						style="position: absolute; bottom: 70px;">
						<page:htmlPage pageNo="${currentPage}" url=""
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" id="BAOCUN" onclick="save()"
					style="display: none;" class="btn btn-primary">保存
				</button>

				<button type="button" id="tuichu" style="display: none;"
					class="btn btn-success" data-dismiss="modal">退出</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-success" data-dismiss="modal">退出
				</button>
			</div>
		</div>
		<!-- /.modal-content -->
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="save1()" class="btn btn-primary">保存
				</button>
