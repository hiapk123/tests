...
<meta http-equiv="description" content="换购促销">
...
function searchGoods(pageIndex,type) {
   ...
   $("div.holder").jPages({
    ...
    typein =saveGoodsToTable(type1);
		...
		$("#hgcx_search_button").attr('onclick','');
		$("#Tsavetotable").attr('onclick', "saveGoodsToTable('"+typein+"');"); 
		$("#hgcx_search_button").attr('onclick', "searchGoods(1,'"+typein+"');");
    	$("#hgcx_all_checkbox").prop("checked",false)
    	var c_id = $("#hgcx_search_select").val();
		var keywords= $("#hgcx_search_input").val(); 
		var str =  ""; 
		var count ="";
	 if(typein!='tianjia'){
    	...
    	//	alert(mydata[0].name)
    		$("#hgcx_tb_list").empty();
			$("#hgcx_tb_list").append(str[0].table);
...
function XGactive(active_type,name,start_time){		
 	...
 	$.post('<%=basePath%>HgcxServlet',
 			...
 			function(str){
 				...
 				if(active_type!="shanchu"){
 						...
 						$("#hidden_name").val($("#mbt_hgcx_active_name").val());
  ...
  function dbClick() {
	...
	function addgoods(type){
		...
		if(t=='object'){
			...
			$("#hgcx_search_button").attr('onclick', "searchGoods(1,'"+type.parent().attr('id')+"');");
		}else if (t=='string'){
			$("#Tsavetotable").attr('onclick', '');
			$("#hgcx_search_button").attr('onclick','');
			$("#Tsavetotable").attr('onclick', "saveGoodsToTable('tianjia');"); 
			$("#hgcx_search_button").attr('onclick', "searchGoods(1,'tianjia');");
		}
		//获得商品分类列表
		$.post('<%=basePath%>HgcxServlet',{"subType":"tanchuceng","tc_type":"getSelect"},function(data){	
			$("#hgcx_search_select").empty();
				$("#hgcx_search_select").append("<option value='0'>--全部分类--</option>");
				for(var i=0;i<data.length;i++){
					$("#hgcx_search_select").append("<option value='"+data[i].c_id+"'>"+data[i].c_name+"</option>");	
				}
				$("#hgcx_tb_list").empty();
 ...
 function checkboxClick(){
		 ...
		 var s = $("#hgcx_all_checkbox").is(':checked');
		 if(s){
			 $(".hgcx_single_checkbox").prop("checked","checked");
			
		 }else{
			  
			 $(".hgcx_single_checkbox").prop("checked",false)
 ...
 function saveGoodsToTable(type){
	  ...
	  sgtt = true;
	  if(type=='tianjia'){
	  var tr_num =  $("#hgcx_daitianjiashangpinxianshi").children("tr[class='tdbody']");
	  var num = tr_num.length+1;
	  var arrChk=$("input[class='hgcx_single_checkbox']:checked");
	  ...
	  g_counts.length=0;
  	   ...
  	   tb_k += "<td id = 'tdminp_"+num+"'><input id = 'minput_"+num+"'   value='0' /></td>";
  	   tb_k +=" <td  id = tddinp_"+num+"'> <input id = 'dinput_"+num+"'  value='0' /></td>";
  	   tb_k +="<td id='glist_"+num+"'>";
  	 	
  	   arrChk.each(function(index,ob){
	  		 var tr_k =$("#hgcx_tr_"+ob.value);
  	   ...
  	   if(arrChk.length!=0){
		 		$("#hgcx_daitianjiashangpinxianshi").append(tb_k);
		 		retype="glist_"+num;
  	   		}
 ...
 function saveGoodsToActive(savetype){
	var trs = $("#hgcx_daitianjiashangpinxianshi").children('tr');
	var info = new Array();
...
//	alert(discount);
   ...
   var hstart_time="";
	...
	var tinfo = info.join('_')
	 ...
	 $.post('<%=basePath%>HgcxServlet',{"subType":"tanchuceng","tc_type":"saveGoods","tinfo":tinfo,"savetype":savetype,
	...
	function backlist(){
		...
		$.post('<%=basePath%>marketing?type=hgcx',
									...
									<div class="modal-dialog" >
										...
										<div class="modal-content">
											<div class="modal-header" >
												...
												搜索关键字：<input id="hgcx_search_input" type="text" value="商品名/条形码" />
												<select id="hgcx_search_select" ><option>全部分类</option></select>
												<button id="hgcx_search_button" class="button" style="margin-left: 20px" type="button" onclick="searchGoods(1,'tianjia');" value="搜索" >搜索</button>
											</div>
											...
											<div class="panel-body ">
													...
													<div class="col-sm-12 col-lg-12" style="max-height: 460px;overflow: scroll;" >
														<table class="col-sm-12 col-lg-12 table tbd" id="hgcx_tb_list"  >
