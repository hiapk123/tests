...
public class shbuyselectbatch extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 查询所需要的信息
		String gettable = "select g_barcode,sa_date,sa_goods_num,sa_goods_price,sa_real_price,sa_discount,s_name from sale a left join store b on a.store_id=b.s_id where a.sa_buyer_id="
				+ v_id + " and sa_date>=" + "'" + time1 + "'" + " and sa_date<=" + "'" + time2 + "'" + " limit 0,10";
		List<Object[]> cmaqsd = new MemInformServiceImp().normalfinad(gettable);
		int currentPage = 1;
		int totalPage = 0;
		String slsql = "select g_barcode,sa_date,sa_goods_num,sa_goods_price,sa_real_price,sa_discount,s_name from sale a left join store b on a.store_id=b.s_id where a.sa_buyer_id="
				+ v_id + " and sa_date>=" + "'" + time1 + "'" + " and sa_date<=" + "'" + time2 + "'";
