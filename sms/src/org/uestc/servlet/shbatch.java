...
public class shbatch extends HttpServlet {
	...
	private void inittable(HttpServletRequest req, HttpServletResponse res) {
		// List<batchgoods> shlist = new
		// TableBatchServiceImp().showtablebatchbyinit();
		// 接收参数
		...
		if ("".equals(shnameofshangpin)) {
			tiaomazhuangtai = 0;
			shlist = new TableBatchServiceImp().yshbathdate(shopname, shzhuangtai, shfenlei, shgonghuoshang,
					currentpage);
			// 定义实现列表显示
			...
			tiaomazhuangtai = 1;
			shlist = new TableBatchServiceImp().mshbathdate(shopname, shzhuangtai, shfenlei, shgonghuoshang,
					shnameofshangpin, currentpage);
			sqlshuliang = new TableBatchServiceImp().yshcount(shopname, shzhuangtai, shfenlei, shgonghuoshang,
					shnameofshangpin);
		}
		...
		// List<batchgoods> shlist = new
		// TableBatchServiceImp().mshowtablebatchbyinit(currentpage);
		// 开始查询商品的数量。
		// ###################################
		/*
		 * String shssll =
		 * "SELECT a.g_id,a.g_name,a.g_barcode,a.c_name,a.g_vip_price,a.su_name,a.g_flag,a.g_integral,a.s_name,b.com_name,a.s_name from goods a LEFT JOIN commission_rule b  ON a.com_id=b.com_id  where a.s_name="
		 * +"'"+shopname+"'"+"and a.g_flag="+shzhuangtai+" and a.c_name="
		 * +shfenlei+" and a.su_name="+"'"+shgonghuoshang+"'"; List<Object[]>
		 * qqqlist=new MemInformServiceImp().normalfinad(shssll);
		 * 
		 * int shgoodsnumbers=0; if(qqqlist!=null&&qqqlist.size()!=0) {
		 * shgoodsnumbers=qqqlist.size(); } req.setAttribute("shgoodsnumbers",
		 * shgoodsnumbers);
		 */
		// #######################
