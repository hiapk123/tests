...
public class MarketingDzytjServiceImp implements MarketingDzytjService {
	...
	public String getPageInit(HttpServletRequest req, HttpServletResponse res) {
		...
		//String sql ="select * from discount where d_name = \'"+name+"\' and d_start_date = \'"+start_time+"\' and s_id = \'"+s_id+"\' and d_type =  1";
		String sql = "select * from discount where  d_end_date >= \'"+time+"\' and active_type = 0  and s_del = 1 ";
		...
		String itmscount ="";
		ArrayList<Map> amap  = SqlHelper.findAll(sql1);
		if(amap==null||amap.size()==0){
			itmscount = "0";
		}else{
			itmscount = amap.get(0).get("count(*)")+"";
		}
		req.setAttribute("pageCount", itmscount);
		return null;
	}
	@Override
	...
	private void progressXG(HttpServletRequest req,HttpServletResponse resp,String type){
		...
		String sql = "select * from discount where d_name = \'"+d_name +"\' and d_start_date = \'"+d_start_time+"\' and s_del =1 and active_type = 0 ";
	...
	private void progressShanchu(HttpServletRequest req ,HttpServletResponse resp){
		String d_name  = (String)req.getParameter("d_name");
		String d_start_time  = (String)req.getParameter("d_start_date");
		String sql = "delete from discount where d_name = \'"+d_name+"\' and d_start_date = \'"+d_start_time+"\' and s_del = 1 and active_type = 0 ";
		String p[]={};
	...
	private StringBuffer saveGoodsToActive(HttpServletRequest req){
		...
		if(!hname.equals("")&&!hstart_time.equals("")){
			String getcid = "select coupons_id from discount where d_name = \'"+hname+"\' and d_start_date = \'"+hstart_time+"\' and active_type =  0";
			ArrayList<Map> clist = SqlHelper.findAll(getcid);
			if(clist!=null&&clist.size()!=0){
				cid = ""+clist.get(0).get("coupons_id");
			}
			String sql  = "delete from discount where  d_name = \'"+hname+"\' and d_start_date = \'"+hstart_time+"\' and active_type =  0";
			String p[] = {};
		...
		for(Iterator it = gids.iterator();it.hasNext();){
			...
			//String par[] = {cid,name,start_time,stop_time,type,code,d,1+"",0+"",s_id+""};
			sql = "insert into discount (coupons_id,d_name,d_start_date,d_end_date,d_type,g_id,d_discount,s_del,active_type,s_id)";	
		
			sql += "values ("+cid+",'"+name+"','"+start_time+"','"+stop_time+"','"+type+"','"+code+"','"+d+"','1','0',s_id)";
			System.out.println("save goods to dzytj_active:");
